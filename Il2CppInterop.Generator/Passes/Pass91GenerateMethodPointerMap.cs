...
using Il2CppInterop.Runtime.Maps;
...
namespace Il2CppInterop.Generator.Passes
{
    public static class Pass91GenerateMethodPointerMap
    {
        public static void DoPass(RewriteGlobalContext context, GeneratorOptions options)
        {
            ...
            using var writer = new BinaryWriter(new FileStream(Path.Combine(options.OutputDir, MethodAddressToTokenMapBase<Assembly, MethodBase>.FileName), FileMode.Create, FileAccess.Write), Encoding.UTF8, false);
            writer.Write(header);
            ...
            if (options.Verbose)
            {
                using var plainTextWriter = new StreamWriter(Path.Combine(options.OutputDir, MethodAddressToTokenMapBase<Assembly, MethodBase>.FileName + ".txt"));
