...
public static class Pass70GenerateProperties
{
        ...
        foreach (var assemblyContext in context.Assemblies)
            foreach (var typeContext in assemblyContext.Types)
            {
                ...
                foreach (var oldProperty in type.Properties)
                {
                    ...
                    var propertyType = assemblyContext.RewriteTypeRef(oldProperty.Signature!.ReturnType, typeContext.isBoxedTypeVariant);
                    ...
                    foreach (var oldParameter in oldProperty.Signature.ParameterTypes)
                        signature.ParameterTypes.Add(assemblyContext.RewriteTypeRef(oldParameter, typeContext.isBoxedTypeVariant));
        ...
        ITypeDefOrRef declaringType, Dictionary<string, int> countsByBaseName)
    {
        ...
        var baseName = "prop_" + assemblyContext.RewriteTypeRef(prop.Signature!.ReturnType, false).GetUnmangledName(prop.DeclaringType);
